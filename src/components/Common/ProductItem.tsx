    ...
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        {item.imgs?.previews?.[0] ? (
          ...
          <div className="flex items-center justify-center w-[250px] h-[250px] bg-gray-200 rounded">
            <span className="text-gray-500">No image</span>
          </div>
        )}

        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-4 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          <button
            onClick={() => {
              openModal();
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-xs sm:text-sm text-gray-600">({item.reviews})</p>
      </div>

      <h3
        className="font-medium text-dark ease-out duration-200 hover:text-blue mb-2 text-sm sm:text-base line-clamp-2 min-h-[2.5rem] sm:min-h-[3rem]"
        onClick={() => handleProductDetails()}
      >
        <Link href="/shop-details" className="block"> {item.title} </Link>
      </h3>
