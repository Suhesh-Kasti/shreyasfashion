...
import DynamicFilters from "../Shop/DynamicFilters";
import DaniosProductCard from "../ProductCard/DaniosProductCard";
import { Product } from "@/types/product";
import { Category } from "@/types/category";
import { getFilteredProducts, getAllCategories } from "../../../lib/sanity";
...
const ShopWithSidebar = ({ products: initialProducts, categories: initialCategories }: ShopWithSidebarProps) => {
  ...
  // Load categories on component mount if not provided
  useEffect(() => {
  ...
  // Filter products when category, search, or dynamic filters change
  useEffect(() => {
    const filterProducts = async () => {
      const hasFilters = selectedCategory || searchQuery || Object.keys(dynamicFilters).length > 0;
      ...
      setIsLoading(true);
      try {
        let filtered = await getFilteredProducts(selectedCategory, searchQuery);
  ...
  const handleDynamicFiltersChange = (filters: Record<string, any>) => {
    setDynamicFilters(filters);
  };

  // Apply dynamic filters client-side
  const applyDynamicFilters = (products: Product[], filters: Record<string, any>): Product[] => {
      ...
      for (const [filterKey, filterValue] of Object.entries(filters)) {
        ...
        if (filterKey === 'gender') {
          ...
          if (Array.isArray(filterValue)) {
            if (!filterValue.some(val => productGender?.includes(val.toLowerCase()))) {
              return false;
            }
          } else {
            if (!productGender?.includes(filterValue.toLowerCase())) {
              return false;
            }
          }
        }
          ...
          if (Array.isArray(filterValue) && product.sizes) {
            const hasMatchingSize = filterValue.some(size =>
          ...
          if (Array.isArray(filterValue) && product.colors) {
            const hasMatchingColor = filterValue.some(color =>
              product.colors?.some(productColor =>
                productColor.name.toLowerCase() === color.toLowerCase() ||
                productColor.value.toLowerCase() === color.toLowerCase()
              )
            );
            if (!hasMatchingColor) {
              return false;
        ...
        else if (filterKey.endsWith('_min')) {
          const priceField = filterKey.includes('price') ? 'discountedPrice' : 'price';
          if (product[priceField] < filterValue) {
            return false;
          }
        }
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex flex-col gap-6">
                  ...
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    ...
                    {(selectedCategory || searchQuery || Object.keys(dynamicFilters).length > 0) && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {selectedCategory && (
                          <span className="inline-flex items-center gap-1 bg-blue/10 text-blue px-3 py-1 rounded-full text-sm">
                            Category: {categories.find(cat => cat.slug === selectedCategory)?.title || selectedCategory}
                            <button
                        ...
                        {Object.entries(dynamicFilters).map(([key, value]) => {
                          ...
                          return (
                            <span key={key} className="inline-flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm">
                              {key}: {displayValue}
                              <button
                                onClick={() => {
            ...
            <div className="xl:max-w-[870px] w-full">
              {/* Search Bar */}
              <div className="rounded-lg bg-white shadow-1 p-4 mb-6">
                <form onSubmit={(e) => {
                  e.preventDefault();
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      ...
                      Products
                      {selectedCategory && (
                        <span className="ml-2 text-blue">in {categories.find(cat => cat.slug === selectedCategory)?.title || selectedCategory}</span>
                      )}
                      {searchQuery && (
                        <span className="ml-2 text-blue">for &ldquo;{searchQuery}&rdquo;</span>
                      )}
                    </p>
                  </div>
                ...
                {isLoading ? (
                  <div className="col-span-full flex justify-center items-center py-20">
                    <div className="text-center">
                      <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-danios-black mx-auto mb-4"></div>
                      <p className="text-danios-text-light">Loading products...</p>
                    </div>
                  </div>
                ) : filteredProducts.length > 0 ? (
                  filteredProducts.map((item, key) => (
                    <DaniosProductCard product={item} key={key} />
                  ))
                ) : (
                  <div className="col-span-full text-center py-20">
                    <p className="text-danios-text-light text-lg mb-4">No products found</p>
                    {(selectedCategory || searchQuery) && (
                      <button
                        onClick={clearAllFilters}
                        className="bg-danios-black text-white px-6 py-2 hover:bg-danios-text transition-colors"
                      >
                        Clear Filters
                      </button>
                    )}
