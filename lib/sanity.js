...
const fallbackProducts = [
  {
    ...
    description: 'Comfortable cargo pants with multiple pockets, perfect for streetwear style',
    category: 'Pants',
    reviews: 24,
    imgs: {
  ...
  },
  {
    ...
    description: 'Cozy oversized sweatshirt made from premium cotton blend',
  ...
  },
  {
    ...
    description: 'Soft and comfortable essential t-shirt for everyday wear',
    category: 'T-Shirts',
    reviews: 45,
    imgs: {
  ...
  },
  {
    ...
    description: 'Versatile slim fit chinos perfect for casual and smart-casual looks',
    category: 'Pants',
    reviews: 28,
    imgs: {
  ...
  },
  {
    ...
    price: 75.0,
    discountedPrice: 59.0,
    description: 'Warm and stylish hoodie pullover with kangaroo pocket',
    category: 'Sweatshirts',
    reviews: 21,
    imgs: {
...
export async function getFilteredProducts(categorySlug = null, searchQuery = null) {
  ...
  try {
    ...
    if (searchQuery) {
      conditions.push(`(title match $searchQuery || description match $searchQuery)`);
...
export async function getProductsWithFilters(filters = {}) {
  ...
  try {
    ...
    if (material) {
      queryConditions.push('material == $material');
      queryParams.material = material;
    }

    if (style) {
      queryConditions.push('style == $style');
      queryParams.style = style;
    }

    if (tags && tags.length > 0) {
      queryConditions.push('count((tags[])[@ in $tags]) > 0');
    ...
    if (search) {
      queryConditions.push('(title match $search || description match $search)');
    ...
    return [
      {
        ...
        ctaText: 'Shop Sale',
        ctaLink: '/shop-with-sidebar?sale=true',
        featured: true
      }
    ];
  }
